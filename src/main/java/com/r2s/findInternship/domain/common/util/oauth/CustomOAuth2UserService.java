...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User,
        ...
        OAuth2UserInfo oAuth2UserInfo = OAuth2UserInfoFactory.getOAuth2UserInfo(
                oAuth2UserRequest.getClientRegistration().getRegistrationId(), oAuth2User.getAttributes(), clientName);
        if (StringUtils.isEmpty(oAuth2UserInfo.getEmail())) {
            throw new CustomException("Email not found from OAuth2 provider");
        }

        // Optional<User> userOptional =
        // Optional.ofNullable(userRepository.findByEmailAndRole(oAuth2UserInfo.getEmail(),
        // roleService.findById(3))
        // . orElseThrow(
        // () -> new
        // InternalServerErrorException(this.messageSource.getMessage("error.checkEmail",
        // null, null)))
        // );
